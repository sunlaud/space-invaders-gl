...
class QuadTreeCell[T <: TreeNode](bounds: Rectangle) {
  ...
  private def findCell(node: T, maxNodesInCell: Int): Option[QuadTreeCell[T]] = {
    ...
    if (isBoundContains(rightCell, node)){
    ...
    //Left
    val frontCell = frontOpt.getOrElse(new QuadTreeCell[T](new Rectangle(this.bounds.x, this.bounds.y, this.bounds.width, this.halfHeight)))
    if (isBoundContains(frontCell, node)){
    ...
    //Left
    val backCell = backOpt.getOrElse(new QuadTreeCell[T](new Rectangle(this.bounds.x, halfHeight, this.bounds.width, this.halfHeight)))
    if (isBoundContains(backCell, node)){
  ...
  private def isBoundContains(cell: QuadTreeCell[T], node: T): Boolean = {
